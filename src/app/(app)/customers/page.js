...
import { fetchCustomers, fetchCustomerFilters, updateCustomerFollowUp } from "@/lib/customers";
...
/* ----------------------- Follow-up modal ----------------------- */
// Lightweight add-follow-up popup for a customer row: status + date + note.
// Saves via agentic_crm/update_customer_follow_up (appends the note, keeps the
// richer log-activity fields untouched).
function FollowUpModal({ c, onClose, onSaved }) {
  const initialDate = String(c.follow_up_date || "").startsWith("0000") ? "" : String(c.follow_up_date || "").slice(0, 10);
  const [followUp, setFollowUp] = useState(normSlug(c.follow_up) || "");
  ...
  async function save() {
    ...
    setError("");
    try {
      const ok = await updateCustomerFollowUp({
      ...
      // Reflect the change locally without a reload. Mirror the backend's
      // timestamped note-append so the "previous notes" stay accurate.
      const patch = { follow_up: followUp, follow_up_date: date };
      if (note.trim()) {
        const stamped = `${nowStamp()} - ${note.trim()}`;
        patch.follow_up_note = c.follow_up_note ? `${c.follow_up_note}\n${stamped}` : stamped;
      }
      onSaved?.(patch);
    } catch {
      setError("Couldn't save. Please try again.");
        ...
        <div className="flex items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-4">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 truncate text-xs text-slate-500">
              {c.customer_name || "Unknown"} · {c.customer_unique_id || `ID ${c.customer_id}`}
            </div>
          </div>
          ...
          <div>
            ...
            {c.follow_up_note && (
              <div className="mt-2 max-h-28 overflow-y-auto rounded-lg border border-amber-200 bg-amber-50/50 p-2.5 text-[11px] leading-relaxed text-slate-600">
                ...
                <div className="whitespace-pre-line">{c.follow_up_note}</div>
              </div>
            )}
          </div>
...
// stored label matches an option in FOLLOWUP_STATUSES when preselecting the dropdown.
function normSlug(v) {
  return String(v || "").toLowerCase().trim().replace(/\s+/g, "-");
}

// Local "YYYY-MM-DD HH:MM:SS" stamp, matching the backend note-append format for
// the optimistic previous-notes preview.
function nowStamp() {
  const d = new Date();
